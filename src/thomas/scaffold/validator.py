...
def validate_destination_path(path: Path) -> None:
    ...
    """
    # Check for invalid characters (null bytes) early
    if "\x00" in str(path):
        raise PathTooLongError(
            "Destination path contains invalid characters (null byte)."
        )

    # Check path length (Windows compatibility) before any filesystem operations
    # Use str(path) to avoid issues with very long paths in path.resolve()
    if len(str(path)) > WINDOWS_PATH_LIMIT:
        raise PathTooLongError(
            f"Destination path exceeds {WINDOWS_PATH_LIMIT} characters (Windows compatibility limit)."
        )

    # Check if path is a file (catches existing files)
    try:
        ...
        # If we can't stat the path due to OS limits, it's too long
        raise PathTooLongError(
            f"Destination path exceeds filesystem limits: {e}"
        ) from e
